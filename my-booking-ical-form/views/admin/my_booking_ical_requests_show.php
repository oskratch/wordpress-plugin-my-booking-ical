<div class="wrap">
	...
	<form method="post" action="">
		<?php wp_nonce_field('mbif_update_status', 'mbif_nonce'); ?>
		<select name="status">
	...
	<ul class="data-booking">
		<li>
			<strong><?php echo __('Reference', 'my_booking_ical_form');?></strong>: <?php echo esc_html(createReferenceRequest($form->reference, $item->entry_date, $item->id));?>
		</li>
		<li>
		...
		<li>
			<strong><?php echo __('First Name', 'my_booking_ical_form');?></strong>: <?php echo esc_html($item->first_name);?>
		</li>
		<li>
			<strong><?php echo __('Last Name', 'my_booking_ical_form');?></strong>: <?php echo esc_html($item->last_name);?>
		</li>
		<li>
			<strong><?php echo __('Email', 'my_booking_ical_form');?></strong>: <a href="mailto:<?php echo esc_attr($item->email);?>"><?php echo esc_html($item->email);?></a>
		</li>
		<li>
			<strong><?php echo __('Phone', 'my_booking_ical_form');?></strong>: <?php echo esc_html($item->phone);?>
		</li>
		<li>
		...
		<li>
			<strong><?php echo __('Guests', 'my_booking_ical_form');?></strong>: <?php echo intval($item->guest_count);?>
		</li>
		<li>
			<strong><?php echo __('Comments', 'my_booking_ical_form');?></strong>: <?php echo esc_html($item->comments);?>
		</li>
	</ul>
	<?php if ($item->summary) { ?>
	<h2 class="title"><?php echo __('Request summary', 'my_booking_ical_form')?></h2>
	<div><?php echo wp_kses_post($item->summary);?></div>
	<?php } ?>
	<div style="margin-top:20px;">
		<a href="<?php echo esc_url(admin_url('admin.php?page=my_booking_ical_requests&form_id=' . intval($item->form_id))); ?>" class="page-title-action"><?php echo __('Back', 'my_booking_ical_form');?></a>
	</div>
</div>
